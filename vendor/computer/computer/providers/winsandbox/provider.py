...
class WinSandboxProvider(BaseVMProvider):
    ...
    async def run_vm(
        ...
        try:
            ...
            # Patch pywinsandbox to fix the logon script bug where it tries to create symlinks
            # at host paths (e.g., C:\Users\frank\...) which don't exist in the sandbox.
            # We monkey-patch _get_logon_script to prepend mkdir commands.
            import sys
            from winsandbox.session import online_session
            
            original_get_logon_script = online_session.OnlineSession._get_logon_script
            
            def patched_get_logon_script(self, server_address_path):
                script = original_get_logon_script(self, server_address_path)
                # Add mkdir commands to create parent directories for symlinks
                if 'mklink /D' in script and script.startswith('cmd.exe /c "'):
                    # Extract the symlink target path (the host path that needs parent dirs)
                    import re
                    mklink_match = re.search(r'mklink /D "([^"]+)"', script)
                    if mklink_match:
                        symlink_path = mklink_match.group(1)
                        parent_path = str(Path(symlink_path).parent)
                        # Prepend mkdir -p equivalent for Windows
                        mkdir_cmd = f'mkdir "{parent_path}" 2>nul'
                        script = script.replace('cmd.exe /c "', f'cmd.exe /c "{mkdir_cmd} & ')
                return script
            
            online_session.OnlineSession._get_logon_script = patched_get_logon_script
            self.logger.info("Patched pywinsandbox logon script generator")
